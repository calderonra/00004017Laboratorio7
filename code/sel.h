...
Matrix createLocalK(int e,mesh &m){
    ...
    float tao,k,lambda,delta,Ae,J,D;
    
    //Componentes de K
    // [ A+K  (C-L) ]
    // [  D   0 ]
    Matrix matrixA,matrixK,matrixC,matrixL,matrixD, matrixLneg;
    ...
    productRealMatrix(tao*J/(24*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,Beta,2,2,6),6,2,6),matrixA);
    ...
    calculateBPrima(BPrima);
    productRealMatrix((J*delta)/(48*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixL);
    productRealMatrix(-1,matrixL,matrixLneg);
    ...
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixC,matrixLneg,6,3));
...
Vector createLocalb(int e,mesh &m){
    ...
    float psi_x = m.getParameter(EXTERNAL_FORCE_X);
    float psi_y = m.getParameter(EXTERNAL_FORCE_Y);
